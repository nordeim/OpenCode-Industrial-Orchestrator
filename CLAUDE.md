...
> **Status:** Phase 3.1 Complete (External Agent Protocol Integrated)
...
│   │   └── service_ports.py         # ExternalAgentPort (EAP)
...
│       └── external_agent_protocol.py # EAP Protocol Schemas
...
│   │   ├── opencode_client.py       # Internal agent client

│   │   └── eap_agent_adapter.py     # External agent adapter (HTTP)
...
└── src/                              # Marketplace UI & Diagnostics
...
| `execute_session(id)` | Execute via internal/external agent |
...
| `register_agent(request)` | Register internal agent |

| `register_external_agent(request)` | Register via EAP (JSON/HTTP) |

| `heartbeat(agent_id)` | Update liveness/load |
...
POST   /api/v1/agents/external/{id}/heartbeat Keep-alive & load status
...
### Phase 3.1: Agent Marketplace (Complete)

- [x] External Agent Protocol (EAP) Specification

- [x] EAP Registration & Heartbeat APIs

- [x] EAPAgentAdapter (Outbound HTTP dispatch)

- [x] Marketplace UI (Browse, Register, Diagnostics)
